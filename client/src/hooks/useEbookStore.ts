...
const initialViewerLayout: ViewerLayout = {
    ...
    VIEWER_HEADER_HEIGHT: 20,
    VIEWER_FOOTER_HEIGHT: 20,
...
const useEbookStore = create<EbookState>((set, get) => ({
    ...
    setCurrentLocation: (location: Page) =>
        set(
            produce((state) => {
                if (!shallow(JSON.stringify(state.currentLocation), JSON.stringify(location))) {
    ...
    setViewerLayout: (layout: ViewerLayout) =>
        set(
            produce((state) => {
                if (!shallow(JSON.stringify(state.viewerLayout), JSON.stringify(layout))) {
